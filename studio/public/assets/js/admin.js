...
function residentRow(resident) {
  ...
  const price = Number(resident.price) > 0 ? `${Number(resident.price).toLocaleString('ru-RU')} ₽` : 'Цена по запросу';
  return `<article class="admin-row"><img src="${esc(resident.heroImage)}" alt=""><div><span class="status status--${esc(resident.availability)}">${statusLabel(resident.availability)}</span><h3>${esc(resident.name)}</h3><p>${esc(collection?.name || 'Коллекция не выбрана')} · ${esc(resident.technique === 'author-series' ? 'Авторская серия' : 'Оригинал ручной лепки')} · ${esc(price)}</p></div><div class="row-actions"><button class="button button--line button--small" data-edit-resident="${esc(resident.id)}">Редактировать</button></div></article>`;
}
