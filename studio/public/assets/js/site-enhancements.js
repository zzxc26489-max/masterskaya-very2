...
function residentLookup(data) {
  ...
  data.residents.forEach((resident) => {
    [resident.id, resident.slug, resident.name, resident.shortName].filter(Boolean).forEach((key) => map.set(normalize(key), resident));
  });
  return map;
}
...
function setResidentImage(image, resident) {
  ...
  image.style.setProperty('--resident-focus-mobile', resident.mobileFocus || (resident.mobileImage ? 'center' : resident.focus || '72% center'));
...
function upgradeCards(data, lookup) {
  document.querySelectorAll('.resident-card').forEach((card) => {
    ...
    const resident = lookup.get(normalize(title));
    if (!resident) return;
...
function upgradeChronicle(data, lookup) {
  ...
  hero.style.setProperty('--hero-focus-mobile', resident.mobileFocus || (resident.mobileImage ? 'center' : resident.focus || '72% center'));
  ...
  const detailCopy = document.querySelector('.resident-detail__copy');
  const detailActions = detailCopy?.querySelector('.cluster');
  if (detailActions && !shell.querySelector('.chronicle-hero__actions')) {
    const actions = detailActions.cloneNode(true);
    actions.classList.add('chronicle-hero__actions');
  ...
  document.querySelectorAll('.resident-detail, .gallery').forEach((section) => {
    ...
    section.style.setProperty('--resident-focus-mobile', resident.mobileFocus || (resident.mobileImage ? 'center' : resident.focus || '72% center'));
  });
  document.querySelectorAll('.resident-detail__image img, .gallery img').forEach((image) => setResidentImage(image, resident));
}

function replaceGenericResidentImages(data, lookup) {
...
async function runUpgrade() {
  ...
  const onChange = () => refreshResponsiveImages();
  mobileQuery.addEventListener?.('change', onChange);
}

runUpgrade().catch(() => {
  // The original site remains fully usable if this progressive visual layer cannot load.
});
